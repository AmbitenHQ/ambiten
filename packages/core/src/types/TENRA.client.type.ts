...
import { TenraConfig } from "./tenraConfig.js";
import { Document } from "./document.js";

/**
 * Configuration options for the TenraClient.
 */
export interface TenraClientConfig {
	/**
...
export interface TenraResolvedClientScope {
  tenantId?: string;
  requestId?: string;
  dbName: string;
...
/**
 * Options for configuring the TenraClient.
 */
export interface TenraClientOptions {
	/**
	...
	/**
	 * Optional configuration for Tenra.
	 */
	config?: TenraConfig;
};
...
/**
 * Represents the TenraClient interface for interacting with MongoDB.
 * @template T - The type of the document in the collection.
 */
export interface TenraClientType<T extends Document> {
	/**
	 * The MongoDB connection URI.
	 */
	uri: string;
