import { AmbitenClient, AmbitenModel, AmbitenSchema } from '../lib-core';
...
describe('AmbitenModel context resolution', () => {
	let mockDb: any;
	let mockCollection: any;
	let mockProvider: any;
	let model: AmbitenModel<TestDoc>;
	...
	beforeEach(() => {
		...
		model = new AmbitenModel<TestDoc>({
			collectionName: 'users',
			schema: new AmbitenSchema<TestDoc>({} as any),
		...
		await AmbitenContext.run(
			...
			async () => {
				await AmbitenContext.run(
					{ collectionName: 'users' },
					async () => {
						expect(AmbitenContext.getTenantId()).toBe('tenantA');
						expect(AmbitenContext.getRequestId()).toBe('req-1');
						expect(AmbitenContext.getCollectionName()).toBe('users');
					}
				);
			}
		);
	...
	it('should reuse existing session inside nested withTransaction', async () => {
		...
		await AmbitenContext.run({ tenantId: 'tenantA' }, async () => {
			await AmbitenContext.withTransaction(async (session1) => {
				await AmbitenContext.withTransaction(async (session2) => {
	...
	it('should resolve collection using dbName from AmbitenContext', async () => {
		...
		const result = await AmbitenContext.run(
			{
				dbName: 'runtime-db',
	...
	it('should expose transaction session through AmbitenContext during withTransaction', async () => {
		const session = {
			startTransaction: jest.fn(),
			commitTransaction: jest.fn().mockResolvedValue(undefined),
			abortTransaction: jest.fn().mockResolvedValue(undefined),
		...
		const operation = jest.fn().mockImplementation(async () => {
			expect(AmbitenContext.get()?.session).toBe(session);
			return 'ok';
		});
