import { createTenraError, ErrorType } from "../utils";
import { TenraClient } from "../lib-core";
import { Collection, Db, MongoClient } from "mongodb";


jest.mock('@tenra/logger', () => {
	const actual = jest.requireActual('@tenra/logger');
 ...
 */
describe('TenraClient', () => {
	let driver: TenraClient;
	...
	beforeEach(async () => {
		driver = new TenraClient({ uri, options: { dbName } });
	...
	it('should throw error if connection is called without uri', async () => {
		...
		const client = new TenraClient({ uri: '', options: { dbName } });
		if (client) {
	...
	it('should log error if getCollection is called without _client', async () => {
		const client = new TenraClient({ uri: uri, options: { dbName } });
	...
	describe('connect', () => {
				...
				connect: async function () {
					try {
						...
						throw createTenraError(
							ErrorType.TENRAConnectionError,
							expectedMessage,
							{
								details: {
									operation: 'connect',
		...
		// 	// Act & Assert
		// 	try {
		// 		const invalidDriver = new TenraClient({ uri: invalidUri, options: { dbName: dbName } });
		// 		await invalidDriver.connect();
		// 		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		// 	} catch (error: any) {
		// 		expect(error.name).toBe(ErrorType.TenraConnectionError);
	...
	describe('getCollection', () => {
		it('should return a MongoDB Collection object when db is defined', async () => {
			...
			const Tenra = {
				...
				getCollection: function <T extends Document>(name: string): Collection<T> {
					if (!this.db) {
						...
						throw createTenraError(
							ErrorType.TENRACollectionError,
							message,
							{
								details: {
			...
			// Act
			const result = await Tenra.getCollection('testCollection');
		...
		// 	const testDriver = new TenraClient({ uri, options: { dbName: "" } });
		// 	// Act & Assert
	...
	describe('TenraClient › useCollection', () => {
	...
	describe('TenraClient › getClusterInfo', () => {
		const uri = 'mongodb://localhost:27017';
		const dbName = 'testdb';
