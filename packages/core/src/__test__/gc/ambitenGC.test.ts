...
describe('AmbitenGC (registry-driven)', () => {
  ...
  it('should run GC on registered models', async () => {
    ...
    AmbitenModelRegistry.registerModel(mockModel);

    const gc = new AmbitenGC({
      enabled: true,
      logResults: true
    });
  ...
  it('should continue when one model fails if continueOnError is true', async () => {
    ...
    AmbitenModelRegistry.registerModel(failedModel);
    AmbitenModelRegistry.registerModel(successfulModel);
