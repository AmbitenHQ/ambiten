...
import configSchema from './tenra.config.schema.json';
...
const DEFAULT_CONFIG_FILENAME = 'tenra.config.json';

function normalizeTenraConfig(rawConfig: any): TenraConfig {
  const cfg = { ...rawConfig };
  ...
  return cfg as TenraConfig;
}

function assertValidTenraConfig(config: TenraConfig): void {
...
export async function loadTenraConfig(configPath?: string): Promise<TenraConfig> {
  ...
  let parsed: TenraConfig;

  try {
    const raw = await fs.readFile(finalPath, 'utf-8');
    parsed = normalizeTenraConfig(JSON.parse(raw));
  } catch (err) {
    throw new Error(`[TENRA] Failed to parse config file: ${err}`);
  }
  ...
  if (!isValid) {
    ...
    throw new Error(`[TENRA] Invalid configuration: ${errors}`);
  }

  assertValidTenraConfig(parsed);
