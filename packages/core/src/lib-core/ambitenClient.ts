...
const AmbitenSymbol = Symbol.for('Ambiten:default');
 ...
 */
export class AmbitenClient implements BootstrapClient {
	...
	constructor(private readonly _opts: AmbitenClientConfig) {
		this._uri = _opts.uri || process.env.MONGO_URI || AmbitenClient.defaultUri;
		this._defaultDbName =
	...
	static init(opts?: Partial<AmbitenClientConfig>): AmbitenClient {
		const key = String(AmbitenSymbol);
		const existing = this.instances.get(key);
		if (existing) {
			return existing as AmbitenClient;
		}

		const client = createAmbitenClientModule({
 ...
 * arguments, active Ambiten runtime context, and client-level defaults.
 *
 * Resolution priority:
 * 1. Explicit method arguments
 * 2. Active Ambiten runtime context
...
export function createAmbitenClientModule(opts: AmbitenClientConfig) {
	return new AmbitenClient(opts);
}
