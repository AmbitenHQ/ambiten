...
import { TenraClient } from './tenraClient';
import { TenraSchema } from './tenraSchema';
...
import { TenraContext, runManualTransaction } from '../context';
 ...
 */
export class TenraModel<T extends Document> {
  private _provider!: DbProvider;
  private _collectionName!: string;
  private _collectionOverride?: Collection<T>;
  private _schema!: TenraSchema<T>;
  private _initialized = false;
  private _defaultCtx?: ModelContext;
  private readonly _modelGCConfig?: TenraModelOptions<T>['gcConfig'];
   ...
   */
  constructor(options: TenraModelOptions<T>) {
    if (!options) {
      const message = 'TenraModel options are required.';
      throw createTenraError(
        ErrorType.NULL_OR_UNDEFINED,
        message,
        { details: { providedOptions: options, typeError: ErrorType.TENRAModelError } }
      );
    }
    ...
    this._schema =
      options.schema ?? new TenraSchema<T>({} as Record<keyof T, any>);
    this._provider = options.provider ?? TenraClient.init();
    ...
    if (this.isGCEnabled() || schemaGCConfig) {
      TenraModelRegistry.registerModel(this);
    }
  ...
  private mergeCtx(ctx?: ModelContext): ModelContext | undefined {
    const runtimeCtx = TenraContext.get();
  ...
  /**
   * Ensures a runtime Tenra context exists before executing an instrumented action.
   *
  ...
  private async resolveDb(ctx?: ModelContext): Promise<Db> {
    ...
    if (!db || typeof (db as any).collection !== 'function') {
      throw new Error(
        `TENRAModel: provider.db() did not return a valid Db instance for collection "${this._collectionName}".`
      );
    }
  ...
  private resolveCollectionName(ctx?: ModelContext): string {
    ...
    if (!collectionName || typeof collectionName !== 'string' || !collectionName.trim()) {
      throw new Error('TENRAModel: collectionName is not configured.');
    }
 ...
 * 3. Active runtime session from TenraContext
 *
 ...
 */
  private resolveSessionStrict(
    ...
    return externalSession ?? ctx?.session ?? TenraContext.getSession();
  }
  ...
  private ensureConfigured(): void {
    ...
    if (!this._provider || typeof this._provider.db !== 'function') {
      throw new Error(
        'TENRAModel: provider is not configured. Provide a valid DbProvider with a db(ctx) method.'
      );
    }
  }
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      if (!this._schema) {
        this._schema = new TenraSchema<T>({} as Record<keyof T, any>);
      }
      ...
      throw createTenraError(
        ...
        `Failed to initialize TenraModel: ${error?.message ?? String(error)}`,
        {
          details: {
            operation: 'init',
   ...
   */
  bind(ctx: ModelContext): TenraModel<T> {
    const clone = Object.create(this) as TenraModel<T>;
   ...
   */
  async registerModel(options: TenraModelOptions<T>): Promise<void> {
    ...
    this._schema = schema ?? new TenraSchema<T>({} as Record<keyof T, any>);
  ...
  private async ensureGCIndex(
    gc: NonNullable<TenraModelOptions<T>['gcConfig']>
  ): Promise<void> {
   ...
   */
  get schema(): TenraSchema<T> {
    return this._schema as TenraSchema<T>;
  }
  ...
  private initMiddleware() {
    ...
    this._schema.pre('create', async (ctx: TenraMiddlewareContext<T>) => {
    ...
    this._schema.post('create', async (ctx: TenraMiddlewareContext<T>) => {
    ...
    this._schema.post('updateOne', async (ctx: TenraMiddlewareContext<T>) => {
    ...
    this._schema.pre('deleteOne', async (ctx: TenraMiddlewareContext<T>) => {
    ...
    this._schema.pre('aggregate', async (ctx: TenraMiddlewareContext<T>) => {
    ...
    this._schema.post('aggregate', async (ctx: TenraMiddlewareContext<T>) => {
   ...
   *
   * This operation runs inside the active Tenra runtime context, is instrumented
 ...
 *
 * This transactional variant runs within the active Tenra runtime context,
 ...
 *
 * The operation:
 * - runs within the active Tenra runtime context
 ...
 */
  ...
  *
  * This operation runs inside the active Tenra runtime context, executes model
  ...
  * semantics.
  *
  * This operation runs within the active Tenra runtime context, is instrumented
  ...
  */
   ...
   * upsert semantics.
   *
   * This transactional variant runs inside the active Tenra runtime context,
  ...
  * before performing the operation. It runs inside the active Tenra runtime
  ...
  */
   ...
   * inside the active Tenra runtime context, is instrumented through
  ...
  * 3. Fall back to {@link TenraContext.withTransaction} to resolve a client
  ...
  */
  private async withTransaction<R>(
    ...
    return TenraContext.run(
      runtimeBase,
      async () => TenraContext.withTransaction(operation)
    );
  }

  /**
 * Executes the provided operation inside a transaction-aware Tenra runtime context.
 *
 * This helper ensures the operation runs with a resolved MongoDB client session
 * and within the active Tenra runtime context for the current tenant,
 ...
 */
   ...
   *
   * This method runs inside the active Tenra runtime context, is instrumented
   * through {@link measureQuery}, executes registered aggregate middlewares,
   ...
   * Tenra runtime context, creates a change stream, and subscribes the provided
  ...
  */
  ...
  private async invalidateCachePatterns(
    ...
    const basePatterns = [
      `tenra:${tenantId}:${dbName}:${collectionName}:find:*`,
      `tenra:${tenantId}:${dbName}:${collectionName}:findOne:*`
    ];
    ...
    const patterns =
      operation === 'aggregate'
        ? [
          ...basePatterns,
          `tenra:${tenantId}:${dbName}:${collectionName}:aggregate:*`
        ]
        : [
          ...basePatterns,
          `tenra:${tenantId}:${dbName}:${collectionName}:aggregate:*`
    ...
    await Promise.all(
      [...new Set(patterns)].map((pattern) =>
        TenraModel.invalidatePattern(pattern)
      )
    );
  }
   ...
   * provided context and applies the pattern within the Tenra cache namespace.
   *
  ...
  async invalidateModelPattern(
    ...
    return this.runWithModelContext(ctx, async () => {
      ...
      const fullPattern = `tenra:${tenantId}:${dbName}:${collectionName}:${pattern.trim()}`;

      return TenraModel.invalidatePattern(fullPattern);
    });
  }
  ...
  async invalidateDocumentCache(
    ...
    return this.runWithModelContext(ctx, async () => {
      ...
      const pattern = `tenra:${tenantId}:${dbName}:${collectionName}:*${String(doc._id)}*`;

      return TenraModel.invalidatePattern(pattern);
    });
  }
   ...
   *
   * This helper retrieves Redis memory and keyspace statistics, along with Tenra
   * cache hit/miss counters. When a tenant identifier is provided, it also counts
   * cache keys scoped to that tenant within the Tenra cache namespace.
   *
   ...
   */
  static async getCacheStats(tenantId?: string): Promise<TenraCacheStats> {
    try {
      ...
      const hitsRaw = (await redis.get('tenra:cache:stats:hits')) ?? '0';
      const missesRaw = (await redis.get('tenra:cache:stats:misses')) ?? '0';
   ...
   *
   * The operation runs inside the active Tenra runtime context, is instrumented
    ...
    return this.runWithModelContext(ctx, async () =>
      measureQuery(
        ...
        async () => {
          ...
          const middlewareCtx = this.buildMiddlewareContext(
            'runGC' as TenraMiddlewareOperation,
            ctx,
            { meta }
          );
          ...
          if (config.softDelete) {
            ...
            if (config.archiveBeforeDelete) {
              ...
              await col.db.collection('Tenra_archives').insertMany(
                archiveDocs,
                session ? { session } : undefined
              );
 ...
 */
  private buildOperationMeta(
    base?: TenraOperationMeta,
    overrides?: Partial<TenraOperationMeta>
  ): TenraOperationMeta {
    return {
      ...(base ?? {}),
      ...(overrides ?? {}),
 ...
 */
  private buildHookPayload(
    operation: TenraMiddlewareOperation,
    ctx?: ModelContext,
    payload: Partial<TenraHookPayload<T>> = {}
  ): TenraHookPayload<T> {
 ...
 */
  private resolveCache(options?: QueryOptions): TenraCacheAdapter | null {
    if (!options?.cache) {
      return null;
    }
    ...
    return {
      get: async <R>(
        ...
        try {
          ...
          if (!cached) {
            await TenraModel.trackCacheMiss();
            return null;
          }

          const parsed = JSON.parse(cached) as R;

          await TenraModel.trackCacheHit();
          return parsed;
        } catch {
          return null;
        }
 ...
 */
  private buildCacheKey(
    ...
    return `tenra:${tenantId}:${dbName}:${collectionName}:${operation}:${serialized}`;
  }
  ...
  private static async trackCacheMiss(): Promise<void> {
    try {
      ...
      pipeline.incr('tenra:cache:stats:misses');
      pipeline.incr(`tenra:cache:stats:misses:${today}`);
      pipeline.expire(`tenra:cache:stats:misses:${today}`, retentionSeconds);
  ...
  private static async trackCacheHit(): Promise<void> {
    try {
      ...
      const pipeline = redis.multi();
      pipeline.incr('tenra:cache:stats:hits');
      pipeline.incr(`tenra:cache:stats:hits:${today}`);
      pipeline.expire(`tenra:cache:stats:hits:${today}`, retentionSeconds);
 ...
 * `find` operations and stores the results using Tenra's scoped cache key
 ...
 */
  ...
  */
    ...
    return this.runWithModelContext(ctx, async () =>
      measureQuery(
        ...
        async () => {
          ...
          const middlewareCtx = this.buildMiddlewareContext(
            'restoreOne' as TenraMiddlewareOperation,
            ctx,
            {
              filter,
              meta
  ...
  private buildMiddlewareContext(
    operation: TenraMiddlewareOperation,
    ctx?: ModelContext,
    payload: Partial<TenraMiddlewareContext<T>> = {}
  ): TenraMiddlewareContext<T> {
    const merged = this.mergeCtx(ctx);
 ...
 */
  ...
  before(
    operation: TenraMiddlewareOperation,
    handler: TenraMiddlewareHandler<T>
  ): this {
  ...
  beforeFind(handler: TenraMiddlewareHandler<T>): this {
    return this.before('find', handler);
  }

  afterFind(handler: TenraMiddlewareHandler<T>): this {
    return this.after('find', handler);
  }

  beforeFindOne(handler: TenraMiddlewareHandler<T>): this {
    return this.before('findOne', handler);
  }

  afterFindOne(handler: TenraMiddlewareHandler<T>): this {
    return this.after('findOne', handler);
  }

  beforeSave(handler: TenraMiddlewareHandler<T>): this {
    return this.before('create', handler);
  }

  afterSave(handler: TenraMiddlewareHandler<T>): this {
    return this.after('create', handler);
  }

  beforeUpdateOne(handler: TenraMiddlewareHandler<T>): this {
    return this.before('updateOne', handler);
  }

  afterUpdateOne(handler: TenraMiddlewareHandler<T>): this {
    return this.after('updateOne', handler);
  }

  beforeDeleteOne(handler: TenraMiddlewareHandler<T>): this {
    return this.before('deleteOne', handler);
  }

  afterDeleteOne(handler: TenraMiddlewareHandler<T>): this {
    return this.after('deleteOne', handler);
  }

  beforeDeleteMany(handler: TenraMiddlewareHandler<T>): this {
    return this.before('deleteMany', handler);
  }

  afterDeleteMany(handler: TenraMiddlewareHandler<T>): this {
    return this.after('deleteMany', handler);
  }

  beforeBulkInsert(handler: TenraMiddlewareHandler<T>): this {
    return this.before('bulkInsert', handler);
  }

  afterBulkInsert(handler: TenraMiddlewareHandler<T>): this {
    return this.after('bulkInsert', handler);
  }

  beforeBulkUpdate(handler: TenraMiddlewareHandler<T>): this {
    return this.before('bulkUpdate', handler);
  }

  afterBulkUpdate(handler: TenraMiddlewareHandler<T>): this {
    return this.after('bulkUpdate', handler);
  }

  beforeAggregate(handler: TenraMiddlewareHandler<T>): this {
    return this.before('aggregate', handler);
  }

  afterAggregate(handler: TenraMiddlewareHandler<T>): this {
    return this.after('aggregate', handler);
  }

  beforeFindOneAndUpdate(handler: TenraMiddlewareHandler<T>): this {
    return this.before('findOneAndUpdate', handler);
  }

  afterFindOneAndUpdate(handler: TenraMiddlewareHandler<T>): this {
    return this.after('findOneAndUpdate', handler);
  }

  beforeFindOneAndDelete(handler: TenraMiddlewareHandler<T>): this {
    return this.before('findOneAndDelete', handler);
  }

  afterFindOneAndDelete(handler: TenraMiddlewareHandler<T>): this {
    return this.after('findOneAndDelete', handler);
  }

  beforeFindOneAndReplace(handler: TenraMiddlewareHandler<T>): this {
    return this.before('findOneAndReplace', handler);
  }

  afterFindOneAndReplace(handler: TenraMiddlewareHandler<T>): this {
    return this.after('findOneAndReplace', handler);
  }

  beforeFindOneAndUpsert(handler: TenraMiddlewareHandler<T>): this {
    return this.before('findOneAndUpsert', handler);
  }

  afterFindOneAndUpsert(handler: TenraMiddlewareHandler<T>): this {
