...
export class AmbitenGC {
  ...
  constructor(private readonly options: GCOptions = {}) {
    ...
    if (!this.enabled) {
      this.log('info', '[AmbitenGC] Garbage Collector is disabled.');
      return;
    }
    ...
    if (options.cron) {
      ...
      this.log('info', `[AmbitenGC] Cron schedule set to: ${options.cron}`);
      return;
    }

    if (options.interval) {
      this.start();
      this.log('info', `[AmbitenGC] Interval set to: ${options.interval}`);
      return;
    }

    this.log('info', '[AmbitenGC] Initialized. Manual run mode enabled.');
  }
  ...
  start(): void {
    ...
    this.log(
      'info',
      `[AmbitenGC] Started GC loop every ${this.intervalMs} ms`
    );
  }
  ...
  stop(): void {
    ...
    this.log('info', '[AmbitenGC] Stopped.');
  }
  ...
  async runOnce(ctx?: ModelContext): Promise<GarbageCollectorRunResult> {
    ...
    if (this.options.logResults) {
      this.log(
        ...
        `[AmbitenGC] Completed. scanned=${result.scanned}, succeeded=${result.succeeded}, failed=${result.failed}`
      );
    }
