...
export class AmbitenCache {
  constructor(private readonly client: AmbitenCacheClient = redis) { }
   ...
   * 3. "default"
   */
  private buildKey(key: string, options: AmbitenCacheOptions = {}): string {
   ...
   */
  async get<T>(key: string, options?: AmbitenCacheOptions): Promise<T | null> {
   ...
   */
  async invalidate(key: string, options?: AmbitenCacheOptions): Promise<void> {
    const namespacedKey = this.buildKey(key, options);
    await this.client.del(namespacedKey);
...
export function createAmbitenCache(client: AmbitenCacheClient = redis): AmbitenCache {
